...
情报会自动打上业务标签，便于按产品域筛选：

- `Buyer`：用户端入口、下单体验、AI 助手、小程序、消费体验。
- `Promo`：活动、补贴、套餐、大促、赛事/季节营销。
- `Merchant`：商家端、经营工具、服务商、培训、闪电仓、前置仓。
- `Driver`：骑手、骑士、众包、运力、履约、配送规则。
- `S&R`：战略、研报、监管、治理、竞争格局。

自动候选由 `auto_update_intel.py` 的 `BUSINESS_TAG_RULES` 识别；手动录入时可以在页面上直接选择业务标签。
